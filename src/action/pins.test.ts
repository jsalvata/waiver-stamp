...
// The `uses:` pin is not the only place the version appears: the guide and the templates
// also name it in the `gh api …/commits/<tag>` one-liner and in the `waiver-version:`
// override example. A release that bumped the pin but not the prose beside it would be
// worse than either alone — a reader cannot tell which of two versions is authoritative.
// So assert the whole set moves together. (This is the check that catches
...
describe('every version-bearing string tracks the released version', () => {
  ...
  const SHAPES = /@v(\d+\.\d+\.\d+)|commits\/v(\d+\.\d+\.\d+)|waiver-version: (\d+\.\d+\.\d+)/g;

  it.each(PINNED)('%s names only the released version', (file) => {
    const version = JSON.parse(read('package.json')).version;
    const found = [...read(file).matchAll(SHAPES)].flatMap((m) => m[1] ?? m[2] ?? m[3] ?? []);
    expect(found.length).toBeGreaterThan(0); // a file that lost its pin entirely
    expect([...new Set(found)]).toEqual([version]);
  });
});

// The sharpest edge in the repo (fixed in #32): the producer action installs the CLI at
// run time, so if it installed an UNPINNED package the adopter's careful `@v1.11.2` would
// pin the action but not the code that computes the verdict — the tool would float with
// npm's `latest` while the adopter believed they were pinned. The default resolves the
// version from the action's own checkout at the pinned ref instead. These guards fail the
// build if anyone unpins it.
describe('the producer action pins the CLI it installs', () => {
  const actionYml = () => read('.github/actions/waiver-stamp/action.yml');

  it('defaults waiver-version to empty (⇒ resolve from the pinned ref), never `latest`', () => {
    expect(actionYml()).toMatch(/waiver-version:[\s\S]*?default: ''/);
    expect(actionYml()).not.toMatch(/waiver-version:[\s\S]*?default: latest/);
  });

  it('resolves that default from its own checkout', () => {
