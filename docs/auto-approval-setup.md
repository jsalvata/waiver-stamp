...
Each caller ships pinned to a release tag (`@v1.25.2`, kept current on every release), so **you
...
`gh api repos/jsalvata/waiver-stamp/commits/v1.25.2 --jq .sha`. Each caller has exactly one
