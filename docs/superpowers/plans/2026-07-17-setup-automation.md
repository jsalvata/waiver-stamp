...
**Rework** (after PR #65's review run reproduced the bug on released 1.24.2): the identity-gated downgrade was dead code — `resolveReviewerLogin` can never return `github-actions[bot]`, because installation tokens (the default `GITHUB_TOKEN` and App tokens alike) 403 on `GET /user` and `GET /app` needs JWT auth, so `me` is `null` on every real Actions run and the doomed APPROVE still went out (the 1.24.2 tests mocked `/user`/`/app` responses installation tokens never produce). Approvability is also not identity-decidable up front (the repo setting "allow GitHub Actions to approve pull requests" can permit the default token). Reworked to attempt the APPROVE and fall back to COMMENT when GitHub refuses it with the 422 — identity-independent, so the App path (identity equally unresolvable) keeps approving.
